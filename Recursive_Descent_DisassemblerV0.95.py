...
#this one has the 32 bit mode stripped and the 64 bit mode is left
class Recursive_Descent_Disassembler:
    ...
    def parse_instruction(self):
        ...
        while self.pointer < len(self.held):
            instruction_byte = self.held[self.pointer:self.pointer + 16] #16 bytes currently if put in self.held will read 16 bits of file then not close ERROR
